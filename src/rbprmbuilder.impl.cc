...
void RbprmBuilder::addLimb(const char* id, const char* limb, const char* effector, const hpp::floatSeq& offset, const hpp::floatSeq& normal, double x, double y,
                           unsigned long samples, const char* heuristicName, double resolution, const char *contactType, double disableEffectorCollision, double grasp) throw (hpp::Error)
{
    if(!fullBodyLoaded_)
        throw Error ("No full body robot was loaded");
    try
...
hpp::floatSeq* RbprmBuilder::getConfigLastGenerateContact() throw (hpp::Error)
{
    try
    {
        model::Configuration_t res = lastStateGenerateContact_.configuration_;
        hpp::floatSeq* dofArray = new hpp::floatSeq();
        dofArray->length(res.rows());
        for(std::size_t i=0; i< res.rows(); ++i)
        {
...
throw (hpp::Error)
{
    ...
    try
    {
        ...
        Configuration_t config_pos = dofArrayToConfig (std::size_t(6), positions);
        Configuration_t config_rot = dofArrayToConfig (std::size_t(8), rotations);
        ...
        for(int i =0; i< 1; ++i)
        {
            ...
            {
                rbprm::RbPrmLimbPtr_t limb = fullBody()->GetLimbs().at(names[j]);
                limbNames.push_back(limb->limb_->name());
                fcl::Transform3f localFrame, globalFrame;
                localFrame.setTranslation(limb->offset_);
                fcl::Vec3f p(config_pos[j*3], config_pos[j*3+1], config_pos[j*3+2]);
                fcl::Quaternion3f r(config_rot[j*4], config_rot[j*4+1], config_rot[j*4+2], config_rot[j*4+3]);
                globalFrame.setQuatRotation(r);
                globalFrame.setTranslation(p);
                std::vector<bool> posConstraints;
                std::vector<bool> rotConstraints;
                posConstraints.push_back(false);posConstraints.push_back(false);posConstraints.push_back(true);
                rotConstraints.push_back(true);rotConstraints.push_back(true);rotConstraints.push_back(false);
                proj->add(core::NumericalConstraint::create (constraints::Position::create("",fullBody()->device_,
                                                                                           limb->effector_,
                                                                                           localFrame,
                                                                                           globalFrame,
                ...
                if(limb->contactType_ == hpp::rbprm::_6_DOF)
                {
                    ...
                    //value_type theta = 2*(value_type(rand()) / value_type(RAND_MAX) - 0.5) * M_PI;
                    //fcl::Matrix3f r = tools::GetZRotMatrix(theta);
                    fcl::Matrix3f rotation = limb->effector_->initialPosition ().getRotation();
                    proj->add(core::NumericalConstraint::create (constraints::Orientation::create("",fullBody()->device_,
                                                                                                  limb->effector_,
                                                                                                  fcl::Transform3f(rotation),globalFrame,
                                                                                                  rotConstraints)));
                }
            }
            ConfigurationPtr_t configptr = shooter->shoot();
        ...
        for(std::vector<std::string>::const_iterator cit = names.begin(); cit !=names.end(); ++cit)
        {
            std::string limbId = *cit;
            rbprm::RbPrmLimbPtr_t limb = fullBody()->GetLimbs().at(*cit);
            tmp.contacts_[limbId] = false;
            tmp.configuration_ = config;
        }
        lastStateGenerateContact_ = tmp;
        //throw (std::runtime_error("could not generate contact configuration after 1 trials"));
        return 0;
    } catch (const std::exception& exc) {
        throw hpp::Error (exc.what ());
    }
